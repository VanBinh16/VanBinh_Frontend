  ...
  >
    <v-card>
      <v-card-title class="headline red darken-2 white--text font-weight-bold">
        {{ $t("manage_type_service_delete_title") }}
      </v-card-title>
      <hr />
      ...
      <div class="pt-3 px-5" style="font-size: 14px">
        <v-row class="ma-0 py-1">
          <v-col class="pa-0" cols="4">
            <span class="label">{{ $t("manage_type_service_code") }}:</span>
          </v-col>
        ...
        <v-row class="ma-0 py-1">
          <v-col class="pa-0" cols="4">
            <span class="label">{{ $t("manage_type_service_name") }}:</span>
          </v-col>
          <v-col class="pa-0" cols="8">
        ...
        <v-row class="ma-0 py-1">
          <v-col class="pa-0" cols="4">
            <span class="label">{{ $t("manage_type_service_notes") }}:</span>
          </v-col>
          <v-col class="pa-0" cols="8">
            <span class="value" style="color: red; font-size: 500">{{
              item.notes
...
export default {
  ...
  methods: {
    deleteAction: async function () {
      try {
        const response = await typeServiceServices.delete({ id: this.item.id });
        const result = response.data;
        if (result && !result.error) {
          this.$SnackBar.show(
            "success",
            this.$t("manage_type_service_delete_success")
          );
          this.$emit("reload-table");
        } else {
          this.$SnackBar.show("error", this.$t("manage_type_service_delete_error"));
        }
      } catch (e) {
        this.$SnackBar.show(
          "error",
